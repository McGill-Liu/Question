...
function initQuiz() {
    ...
    userAnswers = new Array(computerQuestions.length);
    quizFinished = false;
...
function initQuestionSelector() {
    ...
    computerQuestions.forEach((question, index) => {
...
function goToSelectedQuestion() {
    ...
    if (!isNaN(selectedIndex) && selectedIndex >= 0 && selectedIndex < computerQuestions.length) {
...
function showQuestion(index) {
    const question = computerQuestions[index];
    ...
    if (question.type === "单选题") {
        ...
        generateMultipleChoiceOptions(question);
               ...
               question.type === "填空题" || question.type === "判断改错题" ||
               question.type === "计算题") {
...
// 显示答案
function showAnswer() {
    const question = computerQuestions[currentQuestionIndex];
    ...
    if (question.type === "单选题" || question.type === "多选题") {
        isCorrect = checkAnswer(question, userAnswers[currentQuestionIndex]);
        if (isCorrect) {
...
function updateProgress() {
    progressSpan.textContent = `进度: ${currentQuestionIndex + 1}/${computerQuestions.length}`;
}
...
function updateNavigationButtons() {
    ...
    if (currentQuestionIndex === computerQuestions.length - 1) {
...
function showResults() {
    ...
    // 显示得分总结（只统计选择题）
    const totalMultipleChoiceQuestions = computerQuestions.filter(q => 
    ...
    scoreSummary.innerHTML = `
        ...
        <p>主观题（名词解释、简答、论述、应用题等）需要人工评分</p>
    `;
}
